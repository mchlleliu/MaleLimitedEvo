...
# Export PATHS
export PATH_TO_GFF=/plas1/amardeep.singh/Ensembl.Dmel.Genome.Release/gtf/Drosophila_melanogaster.BDGP6.28.99.gtf.gz
export OUT_PATH_FLAT_GFF=/plas1/amardeep.singh/Ensembl.Dmel.Genome.Release/JunctionSeq.files
...
# Generate test specific dispersion estimates and load into count.set.object
count.set.object <- estimateJunctionSeqDispersions(count.set.object, nCores = num_cores)

# Fit the observed dispersions to a regression to create a fitted dispersion
count.set.object <- fitJunctionSeqDispersionFunction(count.set.object)

# Perform the hypothesis tests to test for differential splice junction/exon usage (DEU)
count.set.object <- testForDiffUsage(count.set.object, nCores = num_cores, )
...
jseq.allgenes.body = jseq.allgenes.body[jseq.allgenes.body$expr_female > mappedReadsThreshold & jseq.allgenes.body$expr_male > mappedReadsThreshold,]
jseq.allgenes.head = jseq.allgenes.head[jseq.allgenes.head$expr_female > mappedReadsThreshold & jseq.allgenes.head$expr_male > mappedReadsThreshold,]

# Unique genes with significant DEU
sig.genes.body = unique(jseq.allgenes.body$geneID[jseq.allgenes.body$geneWisePadj < FDRThreshold])
sig.genes.head = unique(jseq.allgenes.head$geneID[jseq.allgenes.head$geneWisePadj < FDRThreshold])
nonsig.genes.body = unique(jseq.allgenes.body$geneID[jseq.allgenes.body$geneWisePadj > FDRThreshold])
nonsig.genes.head = unique(jseq.allgenes.head$geneID[jseq.allgenes.head$geneWisePadj > FDRThreshold])
...
jseq.allgenes.body.remove.low.expression = jseq.allgenes.body[jseq.allgenes.body$expr_female > mappedReadsThreshold | jseq.allgenes.body$expr_male > mappedReadsThreshold, ]
length(unique((jseq.allgenes.body.remove.low.expression[jseq.allgenes.body.remove.low.expression$geneWisePadj < FDRThreshold,])$geneID))

jseq.allgenes.head.remove.low.expression = jseq.allgenes.head[jseq.allgenes.head$expr_female > mappedReadsThreshold | jseq.allgenes.head$expr_male > mappedReadsThreshold, ]
length(unique((jseq.allgenes.head.remove.low.expression[jseq.allgenes.head.remove.low.expression$geneWisePadj < FDRThreshold,])$geneID))
...
junctionseq.results.body.filtered = junctionseq.results.body[junctionseq.results.body$expr_male > mappedReadsThreshold & junctionseq.results.body$expr_female > mappedReadsThreshold ,]
junctionseq.results.head.filtered = junctionseq.results.head[junctionseq.results.head$expr_male > mappedReadsThreshold & junctionseq.results.head$expr_female > mappedReadsThreshold ,]
...
junctionseq.results.body.filtered$sig.hit[junctionseq.results.body.filtered$geneWisePadj <= FDRThreshold] = 1
junctionseq.results.body.filtered$sig.hit[junctionseq.results.body.filtered$geneWisePadj > FDRThreshold] = 0
junctionseq.results.head.filtered$sig.hit[junctionseq.results.head.filtered$geneWisePadj <= FDRThreshold] = 1
junctionseq.results.head.filtered$sig.hit[junctionseq.results.head.filtered$geneWisePadj > FDRThreshold] = 0
...
sdiu.body.fraction.resample = vector(mode = "numeric", length = numResamplingIterations)
non.sdiu.body.fraction.resample = vector(mode = "numeric", length = numResamplingIterations)
sdiu.head.fraction.resample = vector(mode = "numeric", length = numResamplingIterations)
non.sdiu.head.fraction.resample = vector(mode = "numeric", length = numResamplingIterations)


# Resampling
for (i in 1:numResamplingIterations){

...
#SBATCH -o /gpfs/data/GrieshopLab/Karl/Transcriptomics/Output_Messages/MaleLimitedEvo_Reference_Genome.out
#SBATCH -e /gpfs/data/GrieshopLab/Karl/Transcriptomics/Error_Messages/MaleLimitedEvo_Reference_Genome.err
...
OUTPUT_DIR="/gpfs/data/GrieshopLab/Karl/Transcriptomics/References"
...
if [ ! -f "$REF_FILE" ]; then
  ...
  wget 
https://ftp.ensembl.org/pub/release-102/fasta/drosophila_melanogaster/dna/Drosophila_melanogaster.BDGP6.28.dna.toplevel.fa.gz 
-P "$OUTPUT_DIR"
...
if [ ! -f "$GTF_FILE" ]; then
  ...
  wget 
https://ftp.ensembl.org/pub/release-102/gtf/drosophila_melanogaster/Drosophila_melanogaster.BDGP6.28.102.gtf.gz 
-P "$OUTPUT_DIR"
...
if [ ! -d "${OUTPUT_DIR}/STAR" ]; then
  ...
  STAR --runThreadN 12 --runMode genomeGenerate --genomeSAindexNbases 12 --genomeDir "${OUTPUT_DIR}/STAR" 
--genomeFastaFiles "$REF_FILE" --sjdbGTFfile "$GTF_FILE"
else
